...
import com.cn.ailanguage.primarychinese.Activity_and_Fragment.Adapter.Three.ThreeAdapter;
...
public class ThreeActivity extends AppCompatActivity {
    ...
    private TextView wordSum,book_name;
    private ImageView soundIms,back;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        soundIms = findViewById(R.id.sound_word);
        ...
        //书名
        book_name.setText(com.cn.ailanguage.primarychinese.Activity_and_Fragment.Constant.VOAIDNAME);
                ...
                if(button_up.getText().equals("下一题")){
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            // 在子线程中执行初始化音频的操作
                            try {
                                prepare(Constant.WORDBEAN.get(num).getSound());
...
// 将按钮居中
        ...
        soundIms.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // 启动异步线程加载音频
                new Thread(new Runnable() {
        ...
        //初始化音频
        Log.d("xwh771", Constant.WORDBEAN.get(0).getSound());
        prepare(Constant.WORDBEAN.get(0).getSound());
    }
    private void prepare(String sound){
    ...
    private void Choice(WordBean.DataBean.WordsBean wordBean) {
        ...
        set.add(wordBean.getWord());
        int length = com.cn.ailanguage.primarychinese.Activity_and_Fragment.Constant.WORDALL.size();
        while (true) {
            Random random = new Random();
            int i = random.nextInt(length - 1);
            if(com.cn.ailanguage.primarychinese.Activity_and_Fragment.Constant.WORDALL.get(i).getWord()!=null && !com.cn.ailanguage.primarychinese.Activity_and_Fragment.Constant.WORDALL.get(i).getPhonetic().equals(wordBean.getPhonetic())){
                set.add(com.cn.ailanguage.primarychinese.Activity_and_Fragment.Constant.WORDALL.get(i).getWord());
            }
            if (set.size() == 4) {
                break;
            }
        ...
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i).equals(wordBean.getWord())) {
                which = i;
                break;
            }
        }
        threeAdapter = new ThreeAdapter(this, list, wordBean, which);
        threeAdapter.setCallBack(new ThreeAdapter.CallBack() {
            @Override
            public void clickItem(int position) throws IOException {
                threeAdapter.setIschoose(position);
    ...
    private void toast(final String str) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                android.widget.Toast.makeText(ThreeActivity.this, str, android.widget.Toast.LENGTH_SHORT).show();
            }
        });
    }
    @Override
