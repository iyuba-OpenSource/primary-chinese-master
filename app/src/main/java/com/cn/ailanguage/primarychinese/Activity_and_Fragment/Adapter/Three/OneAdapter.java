...
public class OneAdapter extends RecyclerView.Adapter<OneAdapter.ThreeViewHolder> {
    ...
    public OneAdapter.CallBack getCallBack(OneAdapter.CallBack callBack) {
        return OneAdapter.callBack;
    }

    public void setCallBack(OneAdapter.CallBack callBack) {
        this.callBack = callBack;
    }

    private static OneAdapter.CallBack callBack;     //最后得callback接口
    private int which, white_item, flag = 1;

    public OneAdapter(Context context, List<String> list, WordBean.DataBean.WordsBean words, int which) {
    ...
    @Override
    public OneAdapter.ThreeViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(context).inflate(R.layout.three_adapetr, parent, false);
        return new OneAdapter.ThreeViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(@NonNull OneAdapter.ThreeViewHolder holder, @SuppressLint("RecyclerView") int position) {
